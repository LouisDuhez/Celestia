...
/// THEORY
/// ------
/// While the player is airborne, continuously finds the platform that is
/// visually directly below them (screen-space projection) at any world depth,
/// and snaps only the depth component (camForward axis) of the player's
/// position to match it.
///
/// Runs every frame while airborne — there is NO cooldown. The snap is
/// idempotent: if the player is already at the correct depth the delta is
/// below the threshold and nothing moves. This means the player always tracks
/// the correct platform depth in real time, even when jumping over several
...
///   - |depthDelta| > threshold (not already aligned)
/// </summary>
...
public class DepthSnapper : MonoBehaviour
{
    ...
    [Header("Snap Gate")]
    [Tooltip("Minimum depth delta (metres) required to trigger a snap. " +
             "Prevents micro-corrections when already aligned.")]
    ...
    private Quaternion _gizmoOrientation = Quaternion.identity; // never zero-quaternion
    ...
    private void Update()
    {
        // Only run while airborne and not hanging.
    ...
    private void TrySnap()
    {
        ...
        // Only snap to walkable (upward-facing) surfaces.
    ...
    private void SnapToDepth(Vector3 hitPoint, Vector3 camForward)
    {
        ...
        if (Mathf.Abs(depthDelta) < _snapThreshold) return;
    ...
    private void OnDrawGizmos()
    {
        ...
        if (_gizmoOrientation == default) return; // guard against zero-quaternion
